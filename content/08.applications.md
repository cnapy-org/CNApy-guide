...
With "Make scenario feasible" the fluxes defined in the scenario are modified to be consistent with a quadratic program (QP) (and therefore requires a QLP-capable solver like CPLEX, Gurobi or cbc-coinor). The objective function then minimizes the sum of weighted quadratic errors between the scenario fluxes and calculated fluxes where the weights are the absolute scenario flux values. Note that only reactions with scenario fluxes fixed to values unequal to zero enter into the optimization.
